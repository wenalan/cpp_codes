// md5.cpp — glaze-based JSON parsing variant to chase lower latency (100 updates + stats).
...
struct DepthUpdate {
  int64_t U{};
  int64_t u{};
  std::optional<int64_t> pu{};
  std::vector<std::array<std::string, 2>> b;
  std::vector<std::array<std::string, 2>> a;
};
...
template <> struct glz::meta<DepthUpdate> {
  using T = DepthUpdate;
  static constexpr auto value = glz::object(
      "U", &T::U, "u", &T::u, "pu", &T::pu, "b", &T::b, "a", &T::a);
};

template <> struct glz::meta<DepthSnapshot> {
  using T = DepthSnapshot;
  static constexpr auto value = glz::object("lastUpdateId", &T::lastUpdateId,
                                            "bids", &T::bids, "asks", &T::asks);
};
...
template <typename T>
bool parse_with_glaze(std::string_view payload, T &out, std::string &err) {
  if (auto ec = glz::read_json(out, payload); ec) {
    err = glz::format_error(ec, payload);
    return false;
  }
  return true;
}
...
bool fetch_snapshot(const std::string &symbol, OrderBook &out_book,
  ...
  try {
    ...
    DepthSnapshot snap;
    if (!parse_with_glaze(std::string_view{res.body()}, snap, err)) return false;
...
void apply_update_json(std::string_view payload, long long &last_update_id,
  ...
  DepthUpdate upd;
  std::string err;
  if (!parse_with_glaze(payload, upd, err)) {
    throw std::runtime_error(err);
  }

  long long U = upd.U;
  long long u = upd.u;
  long long pu = upd.pu.value_or(0);
  ...
  bid_d.reserve(upd.b.size());
  ask_d.reserve(upd.a.size());
  for (auto &lvl : upd.b) {
