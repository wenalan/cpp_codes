// md2.cpp — optimized variant: fewer string copies, faster number parsing, stats after 100 updates.
...
inline double parse_double(std::string_view sv) {
  ...
  return std::stod(std::string(sv));  // fallback
}
...
bool fetch_snapshot(const std::string &symbol, OrderBook &out_book,
  ...
  try {
    ...
    auto j = json::parse(res.body(), nullptr, false, true);
    if (j.is_discarded()) {
      err = "parse error";
      return false;
    }
    ...
    out_book.asks.clear();
    for (auto &lvl : j["bids"]) {
      auto p_sv = std::string_view(lvl[0].get_ref<const std::string &>());
      auto s_sv = std::string_view(lvl[1].get_ref<const std::string &>());
      double p = parse_double(p_sv);
      double s = parse_double(s_sv);
...
void apply_update_json(std::string_view payload, long long &last_update_id,
                       OrderBook &book) {
  auto j = json::parse(payload, nullptr, false, true);
  if (j.is_discarded()) throw std::runtime_error("parse error");
  ...
  bid_d.reserve(j["b"].size());
  ask_d.reserve(j["a"].size());
  for (auto &lvl : j["b"]) {
    auto p_sv = std::string_view(lvl[0].get_ref<const std::string &>());
    auto s_sv = std::string_view(lvl[1].get_ref<const std::string &>());
    bid_d.emplace_back(parse_double(p_sv), parse_double(s_sv));
  }
  for (auto &lvl : j["a"]) {
    auto p_sv = std::string_view(lvl[0].get_ref<const std::string &>());
    auto s_sv = std::string_view(lvl[1].get_ref<const std::string &>());
    ask_d.emplace_back(parse_double(p_sv), parse_double(s_sv));
  }
